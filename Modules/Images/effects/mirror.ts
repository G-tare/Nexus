...
const command: BotCommand = {
  data: new SlashCommandBuilder()
    .setName('mirror')
    .setDescription('Mirror/flip a user avatar')
    .addUserOption((opt) =>
      opt.setName('user').setDescription('User avatar to mirror').setRequired(true)
    ),

  module: 'images',
  permissionPath: 'images.mirror',
  cooldown: 5,
  ...
  execute: async (interaction: ChatInputCommandInteraction) => {
    ...
    addSectionWithThumbnail(container, `### 🪞 ${targetUser.username}'s Mirrored Avatar`, avatarUrl);
    addText(container, 'Mirror effect requires image processing library. Coming soon!');
