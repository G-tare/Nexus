...
const command: BotCommand = {
  data: new SlashCommandBuilder()
    .setName('vaporwave')
    .setDescription('Slow down and pitch shift for a dreamy vaporwave vibe'),

  module: 'music',
  permissionPath: 'music.effects.vaporwave',
  cooldown: 3,
  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    const result = await toggleFilter(interaction.guildId!, 'vaporwave');
    if (result.error) {
      await interaction.editReply({
    ...
    const embed = successEmbed('Vaporwave', `✅ Vaporwave effect ${statusText}`)
