...
const command: BotCommand = {
  ...
  execute: async (interaction: ChatInputCommandInteraction) => {
    ...
    try {
      ...
      const embed = buildImageEmbed('🐕 Random Dog', imageUrl, config.embedColor);
      await interaction.editReply(v2Payload([embed]));
    } catch (error) {
      logger.error('Error fetching dog:', error);
      await interaction.editReply('❌ Failed to fetch dog image. Try again later.');
    }
  },
};
