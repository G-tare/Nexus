...
const command: BotCommand = {
  data: new SlashCommandBuilder()
    .setName('triggered')
    .setDescription('Add "Triggered" effect to user avatar')
    .addUserOption((opt) =>
      opt.setName('user').setDescription('User to apply effect to').setRequired(true)
    ),

  module: 'images',
  permissionPath: 'images.triggered',
  cooldown: 5,
  ...
  execute: async (interaction: ChatInputCommandInteraction) => {
    ...
    try {
      ...
      const imageUrl = `https://some-random-api.com/canvas/overlay/triggered?avatar=${encodeURIComponent(avatarUrl)}`;
      const embed = buildImageEmbed(`😤 ${targetUser.username} is TRIGGERED!`, imageUrl, config.embedColor);
      await interaction.editReply(v2Payload([embed]));
    } catch (error) {
      logger.error('Error creating triggered overlay:', error);
      await interaction.editReply('❌ Failed to create triggered overlay. Try again later.');
    }
  },
};
