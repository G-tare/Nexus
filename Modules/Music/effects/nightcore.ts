...
const command: BotCommand = {
  data: new SlashCommandBuilder()
    .setName('nightcore')
    .setDescription('Speed up the track with a higher pitch'),

  module: 'music',
  permissionPath: 'music.effects.nightcore',
  cooldown: 3,
  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    const result = await toggleFilter(interaction.guildId!, 'nightcore');
    if (result.error) {
      await interaction.editReply({
    ...
    const embed = successEmbed('Nightcore', `✅ Nightcore effect ${statusText}`)
