...
const command: BotCommand = {
  data: new SlashCommandBuilder()
    .setName('slowmo')
    .setDescription('Slow down the playback without changing pitch'),

  module: 'music',
  permissionPath: 'music.effects.slowmode',
  cooldown: 3,
  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    const result = await toggleFilter(interaction.guildId!, 'slowmode');
    if (result.error) {
      await interaction.editReply({
    ...
    const embed = successEmbed('Slow Mode', `✅ Slow mode ${statusText}`)
