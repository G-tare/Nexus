  ...
  execute: async (interaction: ChatInputCommandInteraction) => {
    try {
      ...
      if (subcommand === 'view') {
        ...
        await interaction.reply({ content: `✅ Channel <#${channel.id}> has been ${isExcluded ? 'removed from' : 'added to'} the exclusion list.`, ephemeral: true });
        ...
        await interaction.reply({ content: `✅ Role <@&${role.id}> has been ${isExcluded ? 'removed from' : 'added to'} the exclusion list.`, ephemeral: true });
      } else if (subcommand === 'threshold') {
        const days = interaction.options.getInteger('days', true);
        await updateActivityConfig(interaction.guildId!, { inactiveThresholdDays: days });
        await interaction.reply({ content: `✅ Inactivity threshold has been set to \`${days} days\`.`, ephemeral: true });
      } else if (subcommand === 'logchannel') {
        const channel = interaction.options.getChannel('channel');
        await updateActivityConfig(interaction.guildId!, { logChannelId: channel?.id || null });
        await interaction.reply({ content: channel ? `✅ Log channel has been set to <#${channel.id}>.` : '✅ Log channel has been disabled.', ephemeral: true });
      ...
      await interaction.reply({ content: '❌ An error occurred while processing the configuration.', ephemeral: true });
    }
  },
};
