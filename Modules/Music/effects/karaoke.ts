...
const command: BotCommand = {
  data: new SlashCommandBuilder()
    .setName('karaoke')
    .setDescription('Remove vocals for a karaoke-style playback'),

  module: 'music',
  permissionPath: 'music.effects.karaoke',
  cooldown: 3,
  ...
  async execute(interaction: ChatInputCommandInteraction): Promise<void> {
    ...
    const result = await toggleFilter(interaction.guildId!, 'karaoke');
    if (result.error) {
      await interaction.editReply({
    ...
    const embed = successEmbed('Karaoke', `✅ Karaoke mode ${statusText}`)
