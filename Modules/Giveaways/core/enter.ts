  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    try {
      ...
      if (!giveaway || giveaway.guildId !== interaction.guild.id) {
        return interaction.reply({ content: `Giveaway with ID ${giveawayId} not found in this server.`, ephemeral: true });
      }
      ...
      return interaction.reply({ content: 'An error occurred while entering the giveaway.', ephemeral: true });
    }
  },
} as BotCommand;
